...
namespace dylan.NET.K
{
	...
	public class DNCompilation
    {
		...
		internal IList<ResourceDescriptor> _resources;
		internal AssemblyName requestedName;
		...
		internal Tuple<Stream, Stream> CompileInMemory()
		{
			if (streams == null) {
				...
				string effectiveName = assemblyName;
				if (requestedName != null)
				{
					effectiveName = requestedName.Name;
				}
				...
				using (StreamWriter sw = new StreamWriter(Path.Combine(Project.ProjectDirectory, "msbuild.dyl"))) {
	        	   ...
	        	   //write out all defines
					...
					//name
					sw.WriteLine($"[assembly: System.Reflection.AssemblyTitle(\"{effectiveName}\")]");
				...
				try {
					...
					if (File.Exists(Path.Combine(Project.ProjectDirectory, $"{effectiveName}.dyl"))) {
						srcFile = $"{effectiveName}.dyl";
					}
					else if (File.Exists(Path.Combine(Project.ProjectDirectory, "project.dyl"))) {
						srcFile = "project.dyl";
					}
					else {
						messages.Add(new DiagnosticMessage($"The file '{effectiveName}.dyl' or 'project.dyl' was not found, compiling 'msbuild.dyl' into an assembly instead! inside project {effectiveName}({effectiveTargetFramework.FullName})", string.Empty, DiagnosticMessageSeverity.Warning));
					}
				...
				var asm = MemoryFS.GetFile(effectiveName + ".dll");
				...
				if (success) {
					...
					Stream syms = null;
					var pdbPath = Path.Combine(Project.ProjectDirectory, effectiveName + PlatformHelper.DebugExtension);
		...
		internal Stream CompileSatelliteInMemory(string satelliteName, string requestedCultureName)
		{
			string effectiveName = satelliteName;
			var lst = new List<Tuple<string, Stream> >();
			var cultureName = requestedCultureName ?? string.Empty;
			var resourcesForCulture = ResourcesForCulture.GetResourcesForCulture(cultureName, _resources);

			if (resourcesForCulture == null || resourcesForCulture.Count<ResourceDescriptor>() == 0)
			{
				return null;
			}

			using (StreamWriter sw = new StreamWriter(Path.Combine(Project.ProjectDirectory, $"msbuild.{cultureName}.dyl")))
			{
				foreach (var reference in MetadataReferences)
				{
					//dlls from nupkgs
				...
				//no debugging support for a satellite assembly
				sw.WriteLine();
				sw.WriteLine("#debug off");
				sw.WriteLine();
			...
			try {
				...
				StreamUtils.UseConsole = false;
				var cd = Environment.CurrentDirectory;
				dylan.NET.Compiler.Program.Invoke(new string[] {"-inmemory", "-cd", Project.ProjectDirectory, $"msbuild.{cultureName}.dyl"});
				Environment.CurrentDirectory = cd;
			}
			catch {
				success = false;
	...
	public class DNProjectReference : IMetadataProjectReference, IDisposable
    {
        ...
        private void EnsureCompiled() {
        	if (_comp.requestedName == null || PlatformHelper.IsResourceNeutralCulture(_comp.requestedName))
            {
				...
				if (!_compiled)
				{
					...
					{
						//will use neutral resources only
			...
			else
			{
				...
				lock (DNCompilation.lockobj)
				{
					cim = _comp.CompileSatelliteInMemory(_comp.requestedName.Name, _comp.requestedName.CultureName);
				}
				_result = _comp.result;
				_finalError = _comp.finalError;
				_cultureasm = cim;
        ...
        public virtual Assembly Load(AssemblyName assemblyName, IAssemblyLoadContext loader)
        {
			_comp.requestedName = assemblyName;
			EnsureCompiled();
        	Assembly asm = null;
            ...
            if (_asm != null) {
            	asm = loader.LoadStream(_asm, _symbols);
	            _asm.Seek(0, SeekOrigin.Begin);
    	  	}
        ...
        public virtual DiagnosticResult EmitAssembly(string path)
        {
			...
			//write satellite assemblies
			foreach (var resourceGrouping in _comp._resources
                .GroupBy(ResourcesForCulture.GetResourceCultureName, StringComparer.OrdinalIgnoreCase))
            {
                if (!string.IsNullOrEmpty(resourceGrouping.Key))
				{
					...
					lock (DNCompilation.lockobj)
					{
						cim = _comp.CompileSatelliteInMemory(_name + ".resources", resourceGrouping.Key);
					}
					_result = _comp.result;
					if (cim != null)
					{
						Directory.CreateDirectory(Path.Combine(path, resourceGrouping.Key));
						using(Stream stream = new FileStream(Path.Combine(path, resourceGrouping.Key, _name + ".resources.dll"), FileMode.Create) )
						{
		            		cim.Seek(0, SeekOrigin.Begin);
		            		cim.CopyTo(stream);
    		        		cim.Dispose();
    		    		}

						//copy directive file made by build tooling
						if (File.Exists($"msbuild.{resourceGrouping.Key}.dyl"))
						{
	    					File.Copy($"msbuild.{resourceGrouping.Key}.dyl", Path.Combine(path, resourceGrouping.Key, "msbuild.dyl"), true);
	    				}
					}
				}
			}
