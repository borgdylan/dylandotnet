...
namespace dylan.NET.K
{
	...
	public class DNCompilation
    {
		...
		private void ErrorH(CompilerMsg cm) {
			...
			var formatted = $"{cm.File}({cm.Line}): error : {cm.Msg} in project {assemblyName}({effectiveTargetFramework.FullName})";
		...
		private void WarnH(CompilerMsg cm) {
			...
			var formatted = $"{cm.File}({cm.Line}): warning : {cm.Msg} in project {assemblyName}({effectiveTargetFramework.FullName})";
			...
			if (streams == null) {
				...
				using (StreamWriter sw = new StreamWriter(Path.Combine(Project.ProjectDirectory, "msbuild.dyl"))) {
	        	   ...
	        	   //write out all defines
	        	    ...
	        	    foreach (string r in Project.Files.ResourceFiles.Keys) {
		            	sw.Write("#embed \"");
	        	        sw.Write(r);
	        	        sw.WriteLine("\"");
		            }
