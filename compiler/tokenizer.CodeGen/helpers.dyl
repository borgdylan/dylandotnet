...
class public static Helpers
    ...
    method public static void CheckAssignability(var t1 as Managed.Reflection.Type, var t2 as Managed.Reflection.Type)
        ...
        if !t1::IsAssignableFrom(t2) andalso #expr(!NullExprFlg orelse Loader::CachedLoadClass("System.ValueType")::IsAssignableFrom(t1)) then
            ...
            StreamUtils::WriteError(ILEmitter::LineNr, 0, ILEmitter::CurSrcFile, string::Format("Slots of type '{0}' cannot be assigned values of type '{1}'.", t1::ToString(), t2::ToString()))
        end if
    end method
    ...
    method public static TypeAttributes ProcessClassAttrs(var attrs as IEnumerable<of Attributes.Attribute>)
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.PartialAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for a class or delegate.")
            end if
            if flg then
                if fir then
                    fir = false
    ...
    method public static TypeAttributes ProcessClassAttrsE(var attrs as IEnumerable<of Attributes.Attribute>)
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.SealedAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for an enum.")
            end if
            if flg then
                if fir then
                    fir = false
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.FamilyAttr then
                ...
                if assem orelse fam orelse foa orelse faa then
                    StreamUtils::WriteError(attr::Line, attr::Column, ILEmitter::CurSrcFile, errs)
                end if
                fam = true
            ...
            elseif attr is Attributes.PinvokeImplAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for a method.")
            end if
            if flg then
                if fir then
                    fir = false
    ...
    method public static FieldAttributes ProcessFieldAttrs(var attrs as IEnumerable<of Attributes.Attribute>)
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.NotSerializedAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for a field.")
            end if
            if flg then
                if fir then
                    fir = false
    ...
    method public static PropertyAttributes ProcessPropAttrs(var attrs as IEnumerable<of Attributes.Attribute>)
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.SpecialNameAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for a property.")
            end if
            if flg then
                if fir then
                    fir = false
    ...
    method public static EventAttributes ProcessEventAttrs(var attrs as IEnumerable<of Attributes.Attribute>)
        ...
        foreach attr in attrs
            ...
            elseif attr is Attributes.SpecialNameAttr then
                ...
                flg = false
                StreamUtils::WriteWarn(attr::Line, attr::Column, ILEmitter::CurSrcFile, i"'{attr::Value}' is not a valid attribute for an event.")
            end if
            if flg then
                if fir then
                    fir = false
    ...
    method public static Managed.Reflection.Type CommitEvalTTok(var tt as TypeTok)
        ...
        if tt is GenericTypeTok then
            ...
            if typ is null then
                StreamUtils::WriteError(gtt::Line, gtt::Column, ILEmitter::CurSrcFile, i"Generic Type {tstr} could not be found!!")
            end if

            foreach pt in pttoks
                temptyp = CommitEvalTTok(pt)
                if temptyp is null then
                    StreamUtils::WriteError(pt::Line, pt::Column, ILEmitter::CurSrcFile, i"Generic Argument {pt::ToString()} meant for Generic Type {typ::ToString()} could not be found!!")
                end if
                lop::Add(temptyp)
            end for
            ...
            if gtt::NestedType isnot null then
                ...
                if typ is null then
                    StreamUtils::WriteError(gtt::NestedType::Line, gtt::NestedType::Column, ILEmitter::CurSrcFile, i"Nested Type {gtt::NestedType::Value} could not be found!")
                else
    ...
    method public static Managed.Reflection.Type[] ProcessParams(var ps as IEnumerable<of Expr>)
        ...
        foreach p in ps
            ...
            if typ is null then
                StreamUtils::WriteError(curp::VarTyp::Line, curp::VarTyp::Column, ILEmitter::CurSrcFile, i"Class '{curp::VarTyp::Value}' is not defined or is not accessible.")
            else
                lt::Add(typ)
            end if
    ...
    method public static void PostProcessParams(var ps as IEnumerable<of Expr>)
        ...
        foreach p in ps
            ...
            SymTable::AddVar(curp::VarName, false, ILEmitter::ArgInd, CommitEvalTTok(curp::VarTyp),ILEmitter::LineNr)
    ...
    method public static void EmitLiteral(var lit as Literal)
        ...
        elseif lit is DecimalLiteral then
            ...
            StreamUtils::WriteError(lit::Line, lit::Column, ILEmitter::CurSrcFile, i"Loading of literals of type '{CommitEvalTTok(lit::LitTyp)::ToString()}' is not yet supported.")
        end if
    end method
    ...
    method public static object LiteralToConst(var lit as Literal)
        ...
        elseif lit is ULongLiteral then
            ...
            StreamUtils::WriteError(lit::Line, lit::Column, ILEmitter::CurSrcFile, i"Literals of type '{CommitEvalTTok(lit::LitTyp)::ToString()}' are not suitable for constants.")
            return null
        end if
    end method
    ...
    method public static Literal ProcessConst(var lit as ConstLiteral)
        ...
        elseif Loader::CachedLoadClass("System.UInt64")::Equals(lit::IntTyp) then
            ...
            StreamUtils::WriteError(lit::Line, lit::Column, ILEmitter::CurSrcFile, i"Loading of constants of internal type '{lit::IntTyp::ToString()}' is not yet supported.")
            return null
        end if
    end method
    ...
    method public static boolean EmitOp(var op as Op, var s as boolean, var emt as boolean, var bo as BranchOptimisation, var lab as Emit.Label)
        ...
        if op is AddOp then
            ...
            elseif emt then
                ...
                elseif OpCodeSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The '+' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        ...
        elseif op is OrOp then
            ...
            elseif emt then
                if EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'or' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is AndOp then
            ...
            elseif emt then
                if EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'and' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is XorOp then
            ...
            elseif emt then
                if EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'xor' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is NandOp then
            ...
            elseif emt then
                ...
                elseif EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'nand' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is NorOp then
            ...
            elseif emt then
                ...
                elseif EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'nor' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is XnorOp then
            ...
            elseif emt then
                ...
                elseif EqSuppFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'xnor' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        ...
        elseif op is LikeOp then
            if emt then
                if StringFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'like' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        elseif op is NLikeOp then
            if emt then
                if StringFlg then
                    ...
                    StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The 'notlike' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
                end if
            end if
        ...
        elseif op is ShrOp then
            ...
            StreamUtils::WriteError(op::Line, op::Column, ILEmitter::CurSrcFile, i"The '{op::ToString()}' operation is undefined for '{LeftOp::ToString()}' and '{RightOp::ToString()}'.")
        end if
        return false
    end method
    ...
    method public static Token SetPopFlg(var t as Token)
        ...
        //TODO: determine whether these warnings should be more accurate location-wise
        if t3 isnot null then
            t3::PopFlg = true
            if t3::Name::MemberAccessFlg then
                StreamUtils::WriteWarn(ILEmitter::LineNr, 0, ILEmitter::CurSrcFile, "MethodCall/NewCall/ObjInitCall statements should end their chain with a method call etc. and not a field load!!")
            end if
            ...
            if mn is gmn as GenericMethodNameTok then
                ...
                foreach gp in gmn::Params
                    ...
                    if genparams[i] is null then
                        StreamUtils::WriteError(gp::Line, gp::Column, ILEmitter::CurSrcFile, string::Format("Generic Argument {0} meant for Generic Method {1} could not be found!!", gp::ToString(), name))
                    end if
                end for
    ...
    method public static ConstInfo ProcessConstValueTok(var tok as Token)
        ...
        elseif tok is Ident then
            ...
            var typ as Managed.Reflection.Type = CommitEvalTTok(new TypeTok(idtnamarr[0]::Value))

            if typ isnot null then
                if  GetExtFld(typ, idtnamarr[1]::Value) isnot null then
                    if Loader::FldLitFlag then
                        return new ConstInfo() {Typ = Loader::FldLitTyp, Value = Loader::FldLitVal}
                    else
                        StreamUtils::WriteError(idtnamarr[1]::Line, idtnamarr[1]::Column, ILEmitter::CurSrcFile, string::Format("Field '{0}' of the class '{1}' is not a constant.", idtnamarr[1]::Value, typ::ToString()))
                    end if
                else
                    StreamUtils::WriteError(idtnamarr[1]::Line, idtnamarr[1]::Column, ILEmitter::CurSrcFile, string::Format("Field '{0}' is not defined/accessible for the class '{1}'.", idtnamarr[1]::Value, typ::ToString()))
                end if
            else
        ...
        if tok is optok as Op then
            ...
            if tok is AddOp then
                if opcf orelse sf then
                    ...
                    StreamUtils::WriteError(tok::Line, tok::Column, ILEmitter::CurSrcFile, i"The '+' operation is undefined for '{lci::Typ::ToString()}' and '{rci::Typ::ToString()}'.")
                end if
            ...
            elseif tok is OrOp then
                if eqf then
                    ...
                    StreamUtils::WriteError(tok::Line, tok::Column, ILEmitter::CurSrcFile, i"The 'or' operation is undefined for '{lci::Typ::ToString()}' and '{rci::Typ::ToString()}'.")
                end if
            elseif tok is AndOp then
                if eqf then
                    return new ConstInfo() {Value = DynamicHelpers::And<of object>(lci::Value, rci::Value), Typ = lci::Typ}
                else
                    StreamUtils::WriteError(tok::Line, tok::Column, ILEmitter::CurSrcFile, i"The 'and' operation is undefined for '{lci::Typ::ToString()}' and '{rci::Typ::ToString()}'.")
                end if
            else
                StreamUtils::WriteError(optok::Line, optok::Column, ILEmitter::CurSrcFile, i"The '{optok::ToString()}' operation is undefined for '{lci::Typ::ToString()}' and '{rci::Typ::ToString()}'.")
            end if
    ...
    method public static ConstInfo ProcessConstExpr(var exp as Expr)
        if exp::Tokens::get_Count() > 0 then
            ...
            StreamUtils::WriteError(ILEmitter::LineNr, 0, ILEmitter::CurSrcFile, "Unexpected empty constant expression.")
        end if
        return null
    end method
